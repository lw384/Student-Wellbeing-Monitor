...
# 添加 src 目录到 Python 路径，以便可以导入 database 模块
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

from database.db import (
    get_all_students,
    get_course_students,
    get_wellbeing_by_week_and_course,
...
class WellbeingService:
    ...
    def _get_student_count(self, module_code: Optional[str]) -> int:
        """
        统计在当前筛选范围内的学生总数：
        - module_code = None  → 所有学生
        - module_code = 某课程 → 该课程里的学生
        """
        if module_code is None:
            rows = get_all_students()
            return len(rows)
        else:
            rows = get_course_students(module_code)
            return len(rows)

    # =========================================================
    # 1️⃣ getDashboardSummary
    # =========================================================
    ...
    def get_dashboard_summary(
        ...
        输入:
            - start_week: 起始周 (含)
            - end_week:   结束周 (含)
            - module_code: 课程 ID（course_id），None 表示全部课程
        ...
        # 1) 查询 wellbeing 原始数据
        rows = get_wellbeing_by_week_and_course(start_week, end_week, module_code)
        # rows: (course_id, student_id, week, stress_level, hours_slept)
        ...
        for course_id, student_id, week, stress, sleep in rows:
        ...
        # 2) 计算问卷参与人数 & 响应率
        total_students = self._get_student_count(module_code)
        responded_count = len(responded_students)
        response_rate = (responded_count / total_students) if total_students > 0 else 0.0
    ...
    def get_stress_sleep_trend(
        ...
        rows = get_wellbeing_by_week_and_course(start_week, end_week, module_code)
        # rows: (course_id, student_id, week, stress, sleep)
        ...
        for w in all_weeks:
            avg_stress = round(stress_sum[w] / stress_cnt[w], 2) if stress_cnt[w] > 0 else 0.0
            avg_sleep = round(sleep_sum[w] / sleep_cnt[w], 2) if sleep_cnt[w] > 0 else 0.0
            items.append(
                {
                    "week": w,
    ...
    def get_risk_students(
        ...
        if module_code is None:
            ...
            student_rows = get_course_students(module_code)
        # student_rows: (student_id, name, course_id)
        student_name_map: Dict[str, str] = {
            str(sid): name for sid, name, _ in student_rows if sid is not None
        }
        ...
        if student_id is not None:
            ...
            if student_id_str not in student_name_map:
                ...
                for sid, name, _ in all_student_rows:
        ...
        for course_id, row_student_id, week, stress, sleep in rows:
            ...
            try:
                ...
                continue
            cid = str(course_id) if course_id is not None else ""
        ...
        for sid, recs in students_to_process:
            ...
            if not (high_risk or potential):
                if student_id is not None:
                    ...
                    avg_sleep = sum(valid_sleeps) / len(valid_sleeps) if valid_sleeps else None
